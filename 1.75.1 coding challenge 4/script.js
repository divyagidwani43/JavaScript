...
document.querySelector('button').addEventListener('click', function () {
    ...
    console.log(split)
    // console.log(text.split("_")[0] + 
    // text.split("_")[1].slice(0, 1).toUpperCase() + 
    // text.split("_")[1].slice(1))
    ...
    for (n of split) {
        ...
        count++;
        // console.log(count)
        const spliting = n.toLowerCase().trim().split("_")
        const message = `${spliting[0]}${spliting[1].slice(0, 1).toUpperCase()}${spliting[1].slice(1)}`;
        console.log(`${message.padEnd(20, " ")}${"✅".repeat(count)}`)
