...
5 dog owners about their dog's age,
and stored the data into an array (one array for each). For now,
they are just interested in knowing 
whether a dog is an adult or a puppy. A dog is an adult if it is
at least 3 years old, and it's a puppy 
...
const checkDogs = function (dogsJulia, dogsKate) {
    ...
    // 1
    const updateJulia = dogsJulia.slice(1, -2)
    // console.log(updateJulia)

    // 2
    const joined = updateJulia.concat(dogsKate)
    console.log(joined)

    joined.forEach(function (value, i, array) {
        // console.log(i, value)
        const str = value >= 3 ? `Dog number ${i + 1} is an adult 🐕, and is ${value} years old` : `Dog number ${i + 1} is still a puppy 🐶`
        console.log(str)
    })
}
